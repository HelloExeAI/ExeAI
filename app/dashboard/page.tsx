...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const eventNotes = currentPage.notes.filter((note: Note) =>
      ['event', 'meeting', 'travel', 'birthday'].includes(note.type)
    );
    ...
    eventNotes.forEach((note: Note) => {
      ...
      const dateMatch = note.content.match(
        /(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{2,4})/
      );
      const timeMatch = note.content.match(
        /(\d{1,2}):(\d{2})\s*(am|pm)?/i
      );
  ...
  // LEFT SIDEBAR: accepts full todo object
  ...
  // RIGHT SIDEBAR: accepts only a content string
  ...
  const todoItems =
    currentPage?.notes.filter((note: Note) => note.type === 'todo') || [];
  ...
  if (loading || status === 'loading') {
    return (
      <div
        ...
        style={{
          background:
            'linear-gradient(135deg, #f5f7ff 0%, #e9ecff 25%, #e5e8ff 50%, #dbe3ff 75%, #d8e0ff 100%)',
        }}
      >
    ...
    >
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex">
          ...
          <div className="frosted-panel" style={{ width: '208px' }}>
            <RightSidebar onAddTodo={handleAddTodoFromString} />
          </div>
        </div>
      </div>
