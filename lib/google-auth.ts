...
export async function getGmailClient(userId: string) {
    ...
    if (isExpired && account.refresh_token) {
        try {
            ...
            await prisma.account.update({
                ...
                data: {
                    ...
                    refresh_token: credentials.refresh_token || account.refresh_token, // Refresh token might not be returned always
                },
            });
        ...
        oauth2Client.setCredentials({
            ...
            refresh_token: account.refresh_token, // Optional but good practice
        });
    } else {
        throw new Error('No access token available');
    }

    // 4. Return Gmail API client
